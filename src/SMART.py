...
class SMART(BaseModel):
    ...
    config: dict
    verbose: bool = True
    conversion_dict: dict = {'gpt-4': 'SWNorth-gpt-4-0613-20231016-3',
    ...
    def _modelname_to_modelid(self, modelname):
        """Converts a model name to a model id."""
       
        return self.conversion_dict[modelname]
    ...
    def extract_hypotheses_and_justifications(self):
        ...
        puts them into a pandas dataframe. Each hypothesis and its justification
        are in separate columns. The number of rows is equal to the number of hypotheses.

        Parameters:
        text (str): A string containing hypotheses and their justifications.

        Returns:
        pd.DataFrame: A dataframe with hypotheses and justifications.
        """
        ...
        # Lists to store hypotheses and justifications
        hypotheses = []
        justifications = []
        ...
        for line in lines:
            if line.startswith('Hypothesis'):
                # Split the line at the first semicolon to separate hypothesis and justification
                hypothesis_part, justification_part = line.split('; ', 1)

                # Extract text after 'Hypothesis' and 'Justification'
                #hypothesis = hypothesis_part
                hypothesis = hypothesis_part.split(': ', 1)[1]
                justification = justification_part.split(': ', 1)[1]
        ...
        # Add subgroups which is a dictionary
        df['Operationalization'] = subgroups
